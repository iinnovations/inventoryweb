...
function jsonstringparser(string){
    // Turns a string with all quotes into an object
    var dataobject = {};
...
function cleanDirtyText(dirtytext) {
    var cleanText = dirtytext.replace(/\./g,'_').replace(/ /g,'_')
    return cleanText
}
...
function setprecision(num, digits) {
	if (isNumber(num)) {
		var roundednumber=parseFloat(num).toFixed(digits);
	}
...
function wsgiCallbackTableData (actionobj) {
	// Get the data
    actionobj.action = actionobj.action || 'gettabledata';
    actionobj.url = actionobj.url || "/wsgireadonly";
    var url = actionobj.url
    actionobj.start = actionobj.start || 0;
