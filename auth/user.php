...
class user {
	protected $config=array( // the settings
		...
		'password'=>array(
			...
			'salt'=>'my inventory salt' // random characters for salting passwords & sessions
		),
		'pages'=>array(
		...
		'site'=>array(
			'admin'=>'Hyak User', // your name
			'email'=>'creese@hyakelectroworks.com', // address to send new account emails from
			'name'=>'PHP Login Script', // site name to display in emails
			'cookie'=>'hyakvmsdata' // cookie name
		)
	);
	...
	private function login(){ // processes the login form
		...
		$db=$this->sqliteusers();
		$q=$db->prepare("SELECT id, name, email, authlevel, accesskeywords, temp FROM users WHERE name=? AND password=?");
		$q->execute(array($name,$password));
		...
		//		$this->fail("You have not activated your account"); // account not activated
			...
			if($sessionsactive>=(int)$sessionsallowed && (int)$sessionsallowed>0)
		...
		$_SESSION['user']['accesskeywords']=$result['accesskeywords'];
		$redirect=(isset($_SESSION['redirect'])?$_SESSION['redirect']:'/');
		unset($_SESSION['redirect']);
		$this->do_action('login',array($result['id'],$result['name'],$result['email']));

		// grab usermetadata from database
		$db=$this->sqlitemeta();
		$q=$db->prepare("SELECT data FROM usermeta WHERE user=?");
		$q->execute(array($name));
		$result=$q->fetch(PDO::FETCH_ASSOC);

		$_SESSION['user']['metadata']=$result['data'];
	...
	protected function sqliteusers(){ // returns the database object for usersdb
		return new PDO('sqlite:'.'/var/www/credentials/inventory/safe.db');
	}
    protected function sqlitemeta(){ // returns the database object for usersdb
		return new PDO('sqlite:'.'/var/www/credentials/inventory/safe.db');
	}
	
	// This is the new dedicated authlog function
	protected function sqliteauthlog(){ // returns the database object for usersdb
		return new PDO('sqlite:'.'/var/www/credentials/inventory/authlog.db');
	}
    ...
    protected function gmail($to,$subject,$body){ // sends a gmail smtp email
        ...
        $mail->Username   = "creese@interfaceinnovations.org"; // SMTP account username
        $mail->Password   = "(sendsomeusefulmail)";        // SMTP account password

        $mail->SetFrom('vmsbot@gmail.com', 'Hyak VMS Mailer'); // FROM
        $mail->AddReplyTo('vmsbot@gmail.com', 'Hyak VMS Mailer'); // Reply TO
	...
	public function login_form(){ // prints the login form
		...
		"\t\t\t".'<fieldset>'."\n".
		"\t".'<label for="name" style="width:35%">Username:</label><input name="name" id="name" type="text" /><br/>'."\n".
		"\t".'<label for="password" style="width:35%">Password:</label><input name="password" id="password" type="password" />'."\n".
		"\t\t\t".'</fieldset>'."\n".
		"\t\t\t".'<fieldset>'."\n".
